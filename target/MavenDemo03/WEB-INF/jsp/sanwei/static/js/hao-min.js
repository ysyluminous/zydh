...
$(document)
		.ready(
				function() {
					...
					accordion_head.first().addClass("fa-rotate-135").next()
							.show("normal");
...
(function($, h, i, j) {
	...
	function EasyTicker(f, g) {
		...
		function start() {
			...
			$(s.opts.controls.toggle).addClass("et-run").html(
					s.opts.controls.stopText)
		}
		function stop() {
			clearInterval(s.timer);
			s.timer = 0;
			$(s.opts.controls.toggle).removeClass("et-run").html(
					s.opts.controls.playText)
		}
		function move(a) {
			var b, eq, appType;
			...
			.ready(
					function() {
						...
						function setStore(items) {
							window.localStorage.setItem("myLinks", JSON
									.stringify(items))
						}
						function initLists() {
							var items = getStoreItems();
							...
							for (var i = 0; i < items.length; i++) {
								...
								li.innerHTML = '<a href="'
										+ items[i].url
										+ '" target="_blank" class="item" data-id="'
										+ items[i].id
										+ '" data-name="'
										+ items[i].name
										+ '"><img src="http://statics.dnspod.cn/proxy_favicon/_/favicon?domain='
										+ items[i].url.replace(
												/https:\/\/|http:\/\//ig, "")
										+ '">' + items[i].name
										+ '</a><i data-target="' + items[i].id
										+ '" class="remove fa fa-times"></i>';
								li.querySelector(".remove").addEventListener(
										"click", onClickRemoveItem);
								li.querySelector("a.item").addEventListener(
										"click", onClickItem);
						...
						function setRecentItems(items) {
							window.localStorage.setItem("recentItems", JSON
									.stringify(items))
						}
						...
						function initRecentItems() {
							...
							for (var i = 0; i < items.length; i++) {
								...
								li.innerHTML = '<a href="'
										+ url
										+ '" target="_blank" class="item" data-name="'
										+ items[i].name
										+ '"><img src="http://statics.dnspod.cn/proxy_favicon/_/favicon?domain='
										+ url.replace(/https:\/\/|http:\/\//ig,
												"") + '">' + items[i].name
										+ '</a><i data-target="'
										+ items[i].name
										+ '"class="remove fa fa-times"></i>';
								li.querySelector(".remove").addEventListener(
										"click", onClickRemoveRecentItem);
								li.querySelector("a.item").addEventListener(
										"click", onClickItem);
